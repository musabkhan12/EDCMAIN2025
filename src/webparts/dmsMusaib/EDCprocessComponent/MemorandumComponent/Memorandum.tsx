...
const MemoContext = ({ props }: any) => {
  ...
  const handleDepartmentChange = async (selectedOption: any) => {
    ...
    if (selectedOption == null) {
      setFormData((prevFormData) => ({
        ...
        memoNo: AllDept.filter(user => user.ADDepartmentName
          ?.split(',')
          .map((d: string) => d.trim())
          .includes(currentUserDept))[0]
          ? `${AllDept.filter(user => user.ADDepartmentName
            ?.split(',')
            .map((d: string) => d.trim())
            .includes(currentUserDept))[0].DepartmentCode}/${String(new Date().getMonth() + 1).padStart(2, '0')}/${formattedMemoSerialNo}`
          : `0/${String(new Date().getMonth() + 1).padStart(2, '0')}/${formattedMemoSerialNo}`,
        memoFileName: AllDept.filter((user: any) => user.ADDepartmentName
          ?.split(',')
          .map((d: string) => d.trim())
          .includes(currentUserDept))[0]
          ? `${AllDept.filter((user: any) => user.ADDepartmentName
            ?.split(',')
            .map((d: string) => d.trim())
            .includes(currentUserDept))[0].DepartmentCode}_${String(new Date().getMonth() + 1).padStart(2, '0')}_${formattedMemoSerialNo}`
          : `0_${String(new Date().getMonth() + 1).padStart(2, '0')}_${formattedMemoSerialNo}`,
      }));
  ...
  const ApiCallFunc = async () => {
    ...
    if (formitemid) {
      ...
      const newItem1 = await getdigitalsignaturerequestbyID(LIST_TITLE_Memo, sp, Number(formitemid));
  ...
  const OnLoadCallFunc = async () => {
    ...
    if (ChangeRequestTemplateType.length > 0) {
      ...
      setFormData((prevFormData) => ({
        ...
        RevisionDate: template.RevisionDate || null,
        IssueDate: template.IssueDate || null,
      }));
    }
  ...
  const updatedigisignnew = async () => {
    let items = await updateDigitalsign(LIST_TITLE_Memo, sp, DigitalsignID);
    if (items) {
                              ...
                              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                                ...
                                >
                                  {(() => {
                                    ...
                                    return folderName === "MemorandumDigitalSignedDocs" ? (
