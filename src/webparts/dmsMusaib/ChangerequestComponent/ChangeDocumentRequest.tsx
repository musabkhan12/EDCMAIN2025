...
const ChangeDocumentRequestContext = ({ props }: any) => {
  ...
  const renderIframeUrl = (fileUrl: string): string => {
    ...
    if (isOfficeFile) {
      return `https://officeindia.sharepoint.com/sites/edcspfx/_layouts/15/WopiFrame.aspx?sourcedoc=${encodeURIComponent(fileUrl)}&action=embedview`;
    }
  ...
  const OpenFile = (obj: any, sts: string) => {
    ...
    const tenantUrl = "https://officeindia.sharepoint.com";
    const docurl = "/sites/edcspfx/ImportantDocuments/SomeFolder/Example.pdf";

    // Extract parent folder from docurl
    const parent = docurl.substring(0, docurl.lastIndexOf("/") + 1);

    // Construct the final redirect URL
    const redirectURLpdf = `${tenantUrl}/sites/edcspfx/ImportantDocuments/Forms/AllItems.aspx?id=${encodeURIComponent(docurl)}&parent=${encodeURIComponent(parent)}&p=true&ga=1`;
