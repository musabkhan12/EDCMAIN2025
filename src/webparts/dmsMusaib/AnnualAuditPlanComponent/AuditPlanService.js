...
export const getDataRoles = async (_sp) => {
  ...
  export const getItemByID = async (_sp, id) => {
    ...
    .select("*,RecommendationType/RecommendationTypeValue,Author/ID,Author/Title,AuditPlanType/AuditPlanType,AuditPlanType/ID,To/ID,To/Title,Cc/ID,Cc/Title,From/ID,From/Title,From/EMail,CCDepartments/ID,CCDepartments/Department,CCDepartments/DepartmentCode,ToDepartments/ID,ToDepartments/Role").expand("RecommendationType,ToDepartments,CCDepartments,Author,AuditPlanType,To,Cc,From")()
...
export const getGeneratedTemplateDoc = async (_sp, itemId) => {
  let results = [];
  try {
    const res = await _sp.web.lists.getByTitle("AnnualAuditPlanDigitalSignedDocs").items
      .select("*,FileRef, FileLeafRef")
    ...
    if (res && res.length > 0) {
      ...
      const res2 = await _sp.web.lists.getByTitle("AnnualAuditPlanGeneratedTemplateDoc").items
        .select("*,FileRef, FileLeafRef")
        .filter(`ListItemID/ID eq ${itemId}`)
        .orderBy("ID", false)
        .top(1)();
