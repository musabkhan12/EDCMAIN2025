...
export interface SearchResultsWithPaginationProps {
    ...
    setPreviewFileUrl: (url: string) => void; // ⬅️ add this
}

export const SearchResultsWithPagination: React.FC<SearchResultsWithPaginationProps> = ({ searchResult, fieldnamesmapping,fieldtypemappings ,  setPreviewFileUrl // ⬅️ receive this
}) => {
    ...
    const handlePreviewClick = (e: React.MouseEvent<HTMLAnchorElement>, path: string) => {
        e.preventDefault();
        setPreviewFileUrl(path); // Do something in current window
     // Make sure the path is converted into a full URL
     let currentUrl = window.location.href;
    //  currentUrl += `?Previewfile=${path}`;
     currentUrl += `?Previewfile`;
    

  // Debug: Show the updated URL


  localStorage.setItem('previewurladvancesearch', path);
        ...
        <div>
            ...
            <div className="row p-2 pt-0">
                {currentItems.map((res, index) => (
                    console.log(`Rendering search result: ${res}`),
                    console.log(`Rendering search result: ${JSON.stringify(res)}`),
                        ...
                        <div style={{border:'1px solid #f37421', borderRadius:'30px'}}  className="card h-100">
                            <div className="card-body">
                                ...
                                <a  href="#" target='_blank'
                                    onClick={(e) => handlePreviewClick(e, res.Path)}>
